...
function getSavedAddress() {
  try {
    ...
    return selectedDeliveryAddress
      ? { ...saved, address: selectedDeliveryAddress }
      : saved;
  } catch {
    return initialForm;
  }
...
export default function Checkout({ onOrderPlaced, onBack, coupon = null }) {
  ...
  const discountRate =
    coupon === "SAVE30" ? 0.3 : coupon === "SAVE20" ? 0.2 : 0;
  ...
  useEffect(() => {
    ...
    if (!addressFocused || query.length < 2) {
  ...
  function selectSuggestedAddress(address) {
    setForm((previous) => ({ ...previous, address: address.label }));
    setAddressSuggestions([]);
    setAddressFocused(false);
    try {
      localStorage.setItem(DELIVERY_ADDRESS_KEY, address.label);
    } catch {
      /* Storage may be unavailable. */
    }
    window.dispatchEvent(
      new CustomEvent("delivery-address-updated", { detail: address.label }),
    );
  }
  ...
  function validate() {
    const errs = [];
    if (!savedAddress || editingAddress)
      errs.push("Please save your delivery address before placing the order.");
  ...
  function saveAddress() {
    ...
    if (!form.address.trim())
      addressErrors.push("Delivery address is required.");
    if (!/^\d{10}$/.test(form.phone))
      addressErrors.push(
        "Phone number must contain exactly 10 digits and numbers only.",
      );
  ...
  return (
    <section className="checkout-page">
      <button
        type="button"
        className="btn btn--link checkout-back"
        onClick={onBack}
      >
        ← Back to menu
      </button>
      <div className="checkout-heading">
        <div>
          ...
          <p>Confirm your details and we’ll get cooking.</p>
        </div>
        <div className="checkout-progress" aria-label="Checkout progress">
          <span className="checkout-progress__done">✓ Cart</span>
          <i></i>
          <span>2 Details</span>
          <i></i>
          <span>3 Done</span>
      ...
      <div className="checkout-layout">
        <div className="checkout checkout--details">
          <div className="checkout-section-title">
            <span>1</span>
            <div>
              <h2>Delivery details</h2>
              <p>Where should we bring your food?</p>
            </div>
          </div>
          <form onSubmit={handleSubmit} className="checkout__form" noValidate>
            {savedAddress && !editingAddress ? (
              <div className="saved-address">
                <div className="saved-address__top">
                  ...
                  <div className="saved-address__person">
              ...
              <>
                ...
                <label className="checkout-address-field">
                  ...
                  />
                  {addressFocused && form.address.trim().length >= 2 && (
                    <div className="checkout-address-suggestions">
                      {addressSearching ? (
                        ...
                        <p>Keep typing to find your address.</p>
                      )}
                    </div>
                  )}
                  </label>
                ...
                <button
                  ...
                  onClick={saveAddress}
                >
                  {savedAddress ? "Save changes" : "Add address"}
                </button>
              </>
            )}
            ...
            {(!savedAddress || editingAddress) && (
              <div className="address-required-note">
                <span>!</span>
                <p>
                  <strong>Save your address to continue</strong>Your order can
                  only be placed after the delivery details above are saved.
                </p>
              </div>
            )}
              ...
              <div className="payment-options">
                ...
                >
                  ...
                  <span>
                    ...
                    <small>Cash or UPI at your door</small>
                  </span>
                </label>
                <label
                  className={
                ...
                >
                  ...
                  <span>
                    ...
                    <small>Secure digital payment</small>
                  </span>
                </label>
              </div>
              {paymentMethod === "online" && (
                <p className="payment-note">
                  You’ll be redirected to secure payment after placing the
                  order.
                </p>
              )}
            </div>
